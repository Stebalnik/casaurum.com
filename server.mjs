...
const STATIC_ASSET_VERSION = "20260604b";
...
const copy = {
  en: {
    ...
    cta: { consult: "Talk to a Design Specialist", measure: "Request a Measurement", collections: "Explore Collections", project: "Submit Project Details", discuss: "Discuss Your Project", start: "Start a Custom Interior Project" },
    form: formCopy("en"),
    home: {
    ...
    legal: { privacy: ["Privacy Policy", "Submitted form data is used to respond to inquiries, evaluate project fit, prepare follow-up communication and improve the website. This editable policy should be reviewed by counsel before launch."], terms: ["Terms of Use", "Website content is provided for general information about CAS AURUM services. Project details, availability, pricing and scope are confirmed only through written communication."] },
  },
  es: {
    ...
    cta: { consult: "Hablar con un Especialista de Diseño", measure: "Solicitar Medición", collections: "Explorar Colecciones", project: "Enviar Detalles del Proyecto", discuss: "Hablar del Proyecto", start: "Iniciar un Proyecto a Medida" },
    form: formCopy("es"),
    home: {
    ...
    legal: { privacy: ["Política de Privacidad", "Los datos enviados se usan para responder consultas y evaluar el proyecto. Este texto editable debe revisarse legalmente antes del lanzamiento."], terms: ["Términos de Uso", "El contenido del sitio es informativo. Alcance, disponibilidad y condiciones se confirman por escrito."] },
  },
  fr: {
    ...
    cta: { consult: "Parler à un Spécialiste Design", measure: "Demander une Prise de Mesures", collections: "Explorer les Collections", project: "Envoyer les Détails du Projet", discuss: "Discuter du Projet", start: "Démarrer un Projet Sur Mesure" },
    form: formCopy("fr"),
    home: {
    ...
    legal: { privacy: ["Politique de Confidentialité", "Les données envoyées servent à répondre aux demandes et évaluer le projet. Ce texte éditable doit être revu juridiquement avant lancement."], terms: ["Conditions d'Utilisation", "Le contenu est informatif. Portée, disponibilité et conditions sont confirmées par écrit."] },
  },
  ru: {
    ...
    cta: { consult: "Проконсультироваться с Дизайн-Специалистом", measure: "Запросить Замер", collections: "Смотреть Коллекции", project: "Отправить Детали Проекта", discuss: "Обсудить Проект", start: "Начать Интерьерный Проект" },
    form: formCopy("ru"),
    home: {
    ...
    legal: { privacy: ["Политика Конфиденциальности", "Данные из форм используются для ответа на запросы и оценки проекта. Текст является редактируемым и должен быть проверен юристом перед запуском."], terms: ["Условия Использования", "Контент сайта носит информационный характер. Объем, доступность и условия подтверждаются письменно."] },
  },
};
...
function heroVideoMedia(lang) {
  ...
  return `<figure class="hero-media hero-video">
    <video autoplay muted loop playsinline preload="metadata" poster="${poster.src}" aria-label="${escapeHtml(localized("CAS AURUM luxury interior video preview", lang))}">
      <source src="/videos/hero/cas-aurum-bespoke-interiors-north-america-light.mp4" type="video/mp4">
...
function collectionDetailPage(route, collection) {
  return `
    ...
    <section class="seo-copy wide">
      <p class="eyebrow">${escapeHtml(localized("Collection preview", route.lang))}</p>
      <h2>${escapeHtml(localized("Visual concepts across North America", route.lang))}</h2>
      <p>${escapeHtml(localized("These scenes show possible material directions, room types and premium interior concepts for the collection. Use them as a starting point for proportion, finish, lighting and custom scope conversations.", route.lang))}</p>
    </section>
    <section class="concept-grid">${collection.projects.map((project, index) => `
      <article class="concept-card">
        <figure class="concept-media">
          ...
          <figcaption class="project-caption"><strong>${escapeHtml(project.location)}</strong><span>${escapeHtml(shortProjectCaption(project.imageCaption, route.lang))}</span></figcaption>
        </figure>
        <div>
          <span>0${index + 1} · ${escapeHtml(project.location)}</span>
  ...
  const pages = {
    en: {
      ...
      terms: {
        ...
        sections: [
          ...
          ["Uploads and Submitted Materials", "If you upload drawings, plans, inspiration images or other materials, you confirm that you have the right to share them with CAS AURUM for review. You grant CAS AURUM permission to use submitted materials internally to evaluate and respond to your inquiry.", "Do not upload confidential, unlawful or third-party materials you are not authorized to share."],
    ...
    es: {
      privacy: {
        ...
        sections: [
          ["Resumen", "CAS AURUM respeta la privacidad de propietarios, diseñadores, arquitectos, constructores, desarrolladores y clientes comerciales que nos contactan sobre interiores a medida, paneles, muebles, carpintería, cocinas y proyectos relacionados.", "Esta política aplica a consultas del sitio y comunicaciones de proyecto en Estados Unidos, Canadá y México. No crea una relación de cliente ni reemplaza un acuerdo escrito."],
          ["Información que Recopilamos", "Al enviar un formulario, podemos recopilar nombre, email, teléfono, código postal, tipo de proyecto, servicio requerido, presupuesto estimado, plazo, mensaje y archivos que decida subir, como imágenes de inspiración, dibujos o planos.", "También recopilamos contexto técnico como URL de origen, idioma, referrer, fecha, hora y parámetros UTM. Los registros básicos del servidor pueden incluir dirección IP, navegador y dispositivo."],
          ["Uso de la Información", "Usamos la información para responder consultas, evaluar el alcance del proyecto, entender ubicación y necesidades, preparar seguimiento, coordinar consultas o mediciones, mejorar el sitio y mantener registros de leads y CRM.", "No vendemos información personal. No usamos dibujos, planos o archivos enviados como portafolio público sin permiso separado."],
          ["Proveedores y Compartición", "Podemos compartir información con proveedores que ayudan con hosting, email, analítica, CRM, seguridad, formularios, almacenamiento de archivos o notificaciones de leads. Estos proveedores deben usar la información solo para prestar servicios a CAS AURUM.", "Si un proyecto requiere coordinación fuera de nuestra zona inmediata, podemos discutir información limitada con socios adecuados de producción, medición, diseño o instalación después de revisar el alcance."],
      ...
      terms: {
        ...
        sections: [
          ...
          ["Materiales Enviados", "Si sube dibujos, planos, imágenes de inspiración u otros materiales, confirma que tiene derecho a compartirlos. Autoriza a CAS AURUM a usarlos internamente para evaluar y responder a su consulta."],
    ...
    fr: {
      privacy: {
        ...
        sections: [
          ["Aperçu", "CAS AURUM respecte la confidentialité des propriétaires, designers, architectes, constructeurs, promoteurs et clients commerciaux qui nous contactent pour des intérieurs sur mesure, panneaux, mobilier, menuiserie, cuisines et projets connexes.", "Cette politique vise les demandes envoyées par le site et les communications de projet aux États-Unis, au Canada et au Mexique. Elle ne crée pas une relation client et ne remplace pas une entente écrite."],
          ["Renseignements Recueillis", "Lorsque vous soumettez un formulaire, nous pouvons recueillir votre nom, courriel, téléphone, code postal, type de projet, service requis, budget, échéancier, message et fichiers téléversés, comme images d'inspiration, dessins ou plans.", "Nous recueillons aussi le contexte technique : URL source, langue, référent, date, heure et paramètres UTM. Les journaux serveur peuvent inclure adresse IP, navigateur et appareil."],
          ["Utilisation", "Nous utilisons les renseignements pour répondre aux demandes, évaluer le projet, comprendre le lieu et la portée, préparer le suivi, organiser une consultation ou une prise de mesures, améliorer le site et maintenir les dossiers de leads et CRM.", "Nous ne vendons pas les renseignements personnels. Nous n'utilisons pas les dessins, plans ou fichiers soumis comme portfolio public sans autorisation séparée."],
          ["Fournisseurs", "Nous pouvons partager des renseignements avec des fournisseurs qui soutiennent l'hébergement, le courriel, l'analytique, le CRM, la sécurité, les formulaires, le stockage de fichiers ou les notifications de leads. Ils doivent utiliser ces renseignements uniquement pour fournir leurs services à CAS AURUM."],
      ...
      terms: {
        ...
        sections: [
          ...
          ["Matériaux Soumis", "Si vous téléversez dessins, plans, images d'inspiration ou autres documents, vous confirmez avoir le droit de les partager. Vous autorisez CAS AURUM à les utiliser en interne pour évaluer et répondre à votre demande."],
    ...
    ru: {
      privacy: {
        ...
        sections: [
          ["Обзор", "CAS AURUM уважает конфиденциальность владельцев недвижимости, дизайнеров, архитекторов, строителей, девелоперов и коммерческих клиентов, которые обращаются к нам по вопросам кастомных интерьеров, панелей, мебели, столярки, кухонь и связанных проектов.", "Эта политика относится к запросам через сайт и проектной коммуникации в США, Канаде и Мексике. Она не создает клиентские отношения и не заменяет письменное соглашение."],
          ["Какие Данные Мы Собираем", "При отправке формы мы можем собирать имя, email, телефон, ZIP или postal code, тип проекта, нужную услугу, бюджет, сроки, сообщение и файлы, которые вы загружаете, например изображения для вдохновения, чертежи или планы.", "Также мы собираем технический контекст: URL источника, язык, referrer, дату, время и UTM-параметры. Базовые серверные журналы могут включать IP-адрес, браузер и устройство."],
          ["Как Мы Используем Данные", "Мы используем данные, чтобы отвечать на запросы, оценивать соответствие проекта, понимать локацию и объем работ, готовить follow-up, планировать консультации или обсуждение замеров, улучшать сайт и вести lead/CRM-записи.", "Мы не продаем персональную информацию. Мы не используем отправленные чертежи, планы или файлы как публичное портфолио без отдельного разрешения."],
          ["Передача Провайдерам", "Мы можем передавать данные сервисным провайдерам, которые помогают с хостингом, email, аналитикой, CRM, безопасностью, формами, хранением файлов или уведомлениями о заявках. Такие провайдеры должны использовать данные только для оказания услуг CAS AURUM.", "Если проект требует координации вне нашей непосредственной зоны работы, мы можем обсудить ограниченную проектную информацию с подходящими партнерами по производству, замерам, дизайну или установке после оценки задачи."],
      ...
      terms: {
        ...
        sections: [
          ...
          ["Отправленные Материалы", "Если вы загружаете чертежи, планы, изображения или другие материалы, вы подтверждаете, что имеете право ими делиться. Вы разрешаете CAS AURUM использовать их внутри компании для оценки и ответа на запрос.", "Не загружайте конфиденциальные, незаконные или чужие материалы, которыми вы не имеете права делиться."],
...
function schemaGraph(route, title, description) {
  ...
  if (route.key === "home") {
    graph.push({
      ...
      duration: "PT12S",
      contentUrl: `${BASE_URL}/videos/hero/cas-aurum-bespoke-interiors-north-america.mp4`,
      embedUrl: `${BASE_URL}/videos/hero/cas-aurum-bespoke-interiors-north-america.mp4`,
    });
  }
  return { "@context": "https://schema.org", "@graph": graph };
}
